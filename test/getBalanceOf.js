...
contract('Remittance', (accounts) => {
  ...
  it('Should update the received amount in the contract correctly', async () => {
    ...
    // Get starting balance of the Bob in Contract.
    let bobContractStartingBalance = await remittanceInstance.getBalanceOf(bob);

    assert.isTrue(bobContractStartingBalance.eq(amount.sub(hundred)), "Amount wasn't correctly received in Bob");
  });
  ...
  it('Should update the withdrawed amount in the contract correctly', async () => {
    ...
    // Get final balance of the Bob in Contract.
    let bobContractEndingBalance = await remittanceInstance.getBalanceOf(bob);

    assert.isTrue(bobContractEndingBalance.eq(zero), "Amount wasn't correctly withdrawn from Bob");
  });

  it('Should only work if address is given', async () => {
    try
    {
      await remittanceInstance.getBalanceOf({from: bob});
    }
    catch (err)
    {
